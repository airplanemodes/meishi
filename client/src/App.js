...
import EditCard from './components/business/editcard';


function App() {

    /* It's important to be updated with the latest version
    of user data stored at the 'user' object and, for this purpose,
    ...
    const ifUserLogged = async() => {
        // invokes the service function and updates the state with response data
        ...
        <SnackbarProvider maxSnack={1} anchorOrigin={{
            ...
            <Router>
                {/* Header gets 'Route' for re-render on url changes, 
                    it's also waiting for the 'user' state to
                    show additional links if the user is business */}
                { user && <Route path="/" component={Header}/> }

                {/* This main section shows only when user state gets updated */}
                { user && <main style={{ minHeight: "78vh" }}>
                    <Switch>
                    ...
                    <ProtectedRoute businessRoute={true} path="/editcard/:id" comp={EditCard}/>
                    {/* 404 comes last */}
